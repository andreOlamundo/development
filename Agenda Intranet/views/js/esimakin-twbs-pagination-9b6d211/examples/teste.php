<?php
if(!$_POST[sql]){$sql="select * formpar";}else{$sql=$_POST[sql];}//se não tiver recebido uma sql por formulário, usa a sql padrão
 
function pag($sql,$np='25'){
	mysql_connect('localhost','root','sdb@debian'); mysql_select_db('information_schema'); //executa a conexão e seleção de banco de dados
