...
$setCatStr = translate ( 'Definir Local' );
$briefStr = translate ( 'Brief Description' );
$catHelpStr = tooltip ( 'location-help' );
$catStr = translate ( 'Locais' );
$editStr = translate ( 'Edit' );
$globalNoteStr = ( $globals_found
  ? translate ( 'Localização Global* não pode ser alterada.' ) : '' );
...
if ( ! empty ( $error ) )
  ...
  echo <<<EOT
    ...
    <form action="set_entry_cat.php" method="post" name="selectcategory">
      <input type="hidden" name="date" value="{$dat}" />
